...
def Get_Page(x):
    Page = requests.get(x, headers=headers, proxies=proxy)
    Page.encoding = "utf-8"
    Page = Page.text
...
def Get_Img(x):
    ...
    save_img = requests.get(ImgUrl[0], headers=headers, proxies=proxy)
...
def Download():
    ...
    r = requests.get(DVLink, stream=True, proxies=proxy)
